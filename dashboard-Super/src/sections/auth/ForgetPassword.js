...
export default function ForgetPassword() {
  ...
  const handleSubmit = (e) => {
    ...
    if (email.length === 0) { 
      ...
      axios.post(`http://localhost:5000/api/user/requestPasswordReset`, { email: email, role: 'Supervisor' } )
      .then(response => { 
        setTimeout(() => {
          setUpdating2('');
          setOpen(false);
  ...
  return (
    <>
      <Typography variant="h4" gutterBottom>Shyiramo Imeli yawe</Typography>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Typography variant="body2" sx={{ mb: 5 }}>Waba wibuka umubare w'ibanga,{''}
          <Link to={'/auth/signin'}> Kanda hano</Link>
        </Typography>
        <LoadingButton fullWidth size="large" type="submit" variant="contained" sx={{marginTop: 5}}>Hohereza</LoadingButton>
      </form>
